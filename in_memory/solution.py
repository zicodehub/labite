            ...
            for node in solution.chemin :
                ...
                if isinstance(node, SupplierModel):
                    ...
                    for order in commandes:
                        ...
                        if DEBUG_PARCOURS: print(f"V{vehicule.id} ({node.name} -> {order.client.name}) , Order {order.id}, Packed {qty_packed}/{order.qty_fixed} in V{vehicule.id} à {node.name} ")
                        ...
                        if qty_packed > 0 :
                            ...
                            if DEBUG_PARCOURS: print(vehicule.name , len(order.batches), " -- ", node_schema.json(), end='\n')
                    ...
                    qty_delivered = 0
                    if DEBUG_PARCOURS: print(f"CLIENT C{node.name} reçu {len(client_holded_orders)} chargements (de {len(set([h.order.id for h in client_holded_orders]))} commandes) du véhicule V{vehicule.id} ")
                    ...
                    if qty_delivered != 0:
                        ...
                        if DEBUG_PARCOURS: print("No for client ", len(client_holded_orders))
