...
class CRUDItem(CRUDBase[Commande, CommandeCreate, CommandeUpdate]):
    def create(self, obj_in: CommandeCreate, db: Session = SessionLocal()) -> Commande:
    ...
    def get_fournisseurs(self, db: Session = SessionLocal()) -> List[models.Fournisseur]:
        return db.query(models.Fournisseur).join(models.Commande).distinct().all()

    def get_clients(self, db: Session = SessionLocal()) -> List[models.Fournisseur]:
        return db.query(models.Client).join(models.Commande).distinct().all()
    
    
    def get_by_fournisseur(self, f: models.Fournisseur, db: Session = SessionLocal()) -> List[models.Commande]:
        return db.query(self.model).filter(
                self.model.fournisseur_id == f.id).all()

    def get_by_client(self, client: models.Client, db: Session = SessionLocal()) -> List[models.Commande]:
        return db.query(self.model).filter(
                self.model.client_id == client.id).all()
    
    def get_fournisseurs_for_client(self, client: models.Client, db: Session = SessionLocal()) -> List[models.Fournisseur]:
        return db.query(models.Fournisseur).join(models.Commande).filter(self.model.client_id == client.id).distinct().all()

    def get_clients_for_fournisseur(self, f: models.Fournisseur, db: Session = SessionLocal()) -> List[models.Client]:
