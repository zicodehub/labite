...
* In the `docker docker-compose.yml` file, remove the whole service / section `frontend`.
...
If you need to use a different Traefik public network name, update it in the `docker docker-compose.yml` files, in the section:
...
For each service that uses a volume (databases, services with uploaded files, etc) you should have a label constraint in your `docker docker-compose.yml` file.
...
Then you need to have those constraints in your `docker docker-compose.yml` file for the services that need to be fixed with each volume.
...
To use and expand that environment variable inside the `docker docker-compose.yml` files you can add the constraints to the services like:
...
note the `${STACK_NAME?Variable not set}`. In the script `./scripts/deploy.sh`, the `docker docker-compose.yml` would be converted, and saved to a file `docker-stack.yml` containing:
...
Building and pushing is done with the `docker docker-compose.yml` file, using the `docker compose` command. The file `docker docker-compose.yml` uses the file `.env` with default environment variables. And the scripts set some additional environment variables as well.

The deployment requires using `docker stack` instead of `docker-swarm`, and it can't read environment variables or `.env` files. Because of that, the `deploy.sh` script generates a file `docker-stack.yml` with the configurations from `docker docker-compose.yml` and injecting the environment variables in it. And then uses it to deploy the stack.
...
docker compose \
# Pass the file that should be used, setting explicitly docker docker-compose.yml avoids the
# default of also using docker compose.override.yml
-f docker docker-compose.yml \
# Use the docker compose sub command named "config", it just uses the docker docker-compose.yml
...
And there's also a `docker compose.override.yml` with overrides for development, for example to mount the source code as a volume. It is used automatically by `docker compose` to apply overrides on top of `docker docker-compose.yml`.
...
One way to do it could be to add each environment variable to your CI/CD system, and updating the `docker docker-compose.yml` file to read that specific env var instead of reading the `.env` file.
